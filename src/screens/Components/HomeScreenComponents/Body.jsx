...
import { MdSpeed, MdOutlineSubscriptions, MdOutlineLogout, MdOutlineKeyboardBackspace } from "react-icons/md";
...
useEffect(() => {
  ...
  return (
    ...
    <div className="flex">
     ...
     <button className="w-44 border rounded-full"><img src={craft} alt="" /></button>
     <h3 className="flex text-3xl font-semibold">{userD.fName+" "+userD.lName} <button className="ml-4 mt-4"><HiPencil size={18}/></button></h3>
  ...
  <div className="w-screen flex flex-col flex-wrap h-auto">
    <div className="">
      <div className="w-auto p-0 sm:p-6 bg-violet-50">
        ...
        <MdOutlineKeyboardBackspace className="text-white hidden sm:flex" size={20}/> 
        <div className="mt-12 font-sans font-semibold text-base sm:text-2xl lg:text-4xl ">
          Welcome John
          <p className="text-xs lg:text-sm font-sans font-normal  mt-2 w-44 sm:w-auto ">Choose a goal and start learning top educators</p>
        </div> 
        </div>
        <div className="ml-auto">
        <img className=" p-6 self-end w-44 sm:w-auto mt-4 sm:mt-0 " src={girl} alt="" />
        </div>
        </div><div className="flex justify-center items-center">
        <input className="  text-black relative  bottom-2 w-full mx-4 sm:hidden p-3   " type="text" name="search" id="" placeholder="Search"/>
        </div>
